...
int target_fuzz_case_idx = -1;

void init_case_info_mmap_link() {
  int fd;
  if ((fd = open(".case_info_file", O_RDWR | O_CREAT | O_TRUNC,
                 S_IRWXU | S_IRWXG | S_IRWXO)) < 0) {
    FATAL("open case_info_file failed");
  }
  ftruncate(fd, sizeof(struct CaseInfo));
  case_info_mm = (struct CaseInfo *)mmap(
      NULL, sizeof(struct CaseInfo), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
...
void load_queue_entry_from_case_info_mm_by_id() {
  int queue_idx = case_info_mm->modify_queue_entry_idx;
  struct case_info_queue_entry *tmp = &case_info_mm->modify_queue_entry;
  struct queue_entry *p = queue;
  while (queue_idx > 0 && p != NULL) {
    p = p->next;
    queue_idx--;
  }
  p->favored = tmp->favored;
...
#define SP20    SP10 SP10
...
#if CODE_CHECK_IMPL
  sprintf(case_info_mm->fuzz_stat.run_time, "%s", DTD(cur_ms, start_time));
  sprintf(case_info_mm->fuzz_stat.cycles_done, "%s", DI(queue_cycle - 1));
#endif
  ...
  if (!dumb_mode && (last_path_time || resuming_fuzz || queue_cycle == 1 ||
...
#if CODE_CHECK_IMPL
    sprintf(case_info_mm->fuzz_stat.last_new_path, "%s",
            DTD(cur_ms, last_path_time));
#endif
  } else {
    ...
    if (dumb_mode) {
      ...
      sprintf(case_info_mm->fuzz_stat.last_new_path, "%s", "n/a");
    } else {
      SAYF(bV bSTOP "   last new path : " cRST "none yet " cLRD
                    "(odd, check syntax!)      ");
      sprintf(case_info_mm->fuzz_stat.last_new_path, "%s", "none yet");
    }
#else
      if (dumb_mode)
  ...
  SAYF(bSTG bV bSTOP "  total paths : " cRST "%-5s  " bSTG bV "\n",
       DI(queued_paths));
#if CODE_CHECK_IMPL
  sprintf(case_info_mm->fuzz_stat.total_path, "%s", DI(queued_paths));
#endif
  ...
  SAYF(bV bSTOP " last uniq crash : " cRST "%-34s " bSTG bV bSTOP
       ...
       tmp);
#if CODE_CHECK_IMPL
  sprintf(case_info_mm->fuzz_stat.last_uniq_crash, "%s",
          DTD(cur_ms, last_crash_time));
  sprintf(case_info_mm->fuzz_stat.uniq_crashes, "%s", tmp);
#endif
...
#if CODE_CHECK_IMPL
  sprintf(case_info_mm->fuzz_stat.last_uniq_hang, "%s",
          DTD(cur_ms, last_hang_time));
  sprintf(case_info_mm->fuzz_stat.uniq_hangs, "%s", tmp);
#endif
...
#if CODE_CHECK_IMPL
  sprintf(case_info_mm->fuzz_stat.now_processing, "%s", tmp);
#endif
...
#if CODE_CHECK_IMPL
  sprintf(case_info_mm->fuzz_stat.map_density, "%s", tmp);
#endif
...
#if CODE_CHECK_IMPL
  sprintf(case_info_mm->fuzz_stat.paths_timed_out, "%s", tmp);
#endif
...
#if CODE_CHECK_IMPL
  sprintf(case_info_mm->fuzz_stat.count_coverage, "%s", tmp);
#endif
...
#if CODE_CHECK_IMPL
  sprintf(case_info_mm->fuzz_stat.now_trying, "%s", stage_name);
  sprintf(case_info_mm->fuzz_stat.favored_paths, "%s", tmp);
#endif
  if (!stage_max) {
...
#if CODE_CHECK_IMPL
  sprintf(case_info_mm->fuzz_stat.stage_execs, "%s", tmp);
#endif
...
#if CODE_CHECK_IMPL
  sprintf(case_info_mm->fuzz_stat.new_edges_on, "%s", tmp);
#endif
...
#if CODE_CHECK_IMPL
  sprintf(case_info_mm->fuzz_stat.total_execs, "%s", DI(total_execs));
  sprintf(case_info_mm->fuzz_stat.total_crashes, "%s", tmp);
#endif
...
#if CODE_CHECK_IMPL
    sprintf(case_info_mm->fuzz_stat.exec_speed, "%s", tmp);
#endif
...
#if CODE_CHECK_IMPL
  sprintf(case_info_mm->fuzz_stat.total_tmouts, "%s", tmp);
#endif
  ...
  SAYF(bV bSTOP "   bit flips : " cRST "%-37s " bSTG bV bSTOP "    levels : "
       cRST "%-10s " bSTG bV "\n", tmp, DI(max_depth));
#if CODE_CHECK_IMPL
  sprintf(case_info_mm->fuzz_stat.bit_flips, "%s", tmp);
  sprintf(case_info_mm->fuzz_stat.levels, "%s", DI(max_depth));
#endif
  if (!skip_deterministic)
    sprintf(tmp, "%s/%s, %s/%s, %s/%s",
...
#if CODE_CHECK_IMPL
  sprintf(case_info_mm->fuzz_stat.byte_flips, "%s", tmp);
  sprintf(case_info_mm->fuzz_stat.pending, "%s", DI(pending_not_fuzzed));
#endif
  if (!skip_deterministic)
    sprintf(tmp, "%s/%s, %s/%s, %s/%s",
...
#if CODE_CHECK_IMPL
  sprintf(case_info_mm->fuzz_stat.arithmetics, "%s", tmp);
  sprintf(case_info_mm->fuzz_stat.pend_fav, "%s", DI(pending_favored));
#endif
  if (!skip_deterministic)
    sprintf(tmp, "%s/%s, %s/%s, %s/%s",
...
#if CODE_CHECK_IMPL
  sprintf(case_info_mm->fuzz_stat.known_inis, "%s", tmp);
  sprintf(case_info_mm->fuzz_stat.own_finds, "%s", DI(queued_discovered));
#endif
  if (!skip_deterministic)
    sprintf(tmp, "%s/%s, %s/%s, %s/%s",
...
#if CODE_CHECK_IMPL
  sprintf(case_info_mm->fuzz_stat.dictionary, "%s", tmp);
  sprintf(case_info_mm->fuzz_stat.imported, "%s",
          sync_id ? DI(queued_imported) : (u8 *)"n/a");
#endif
  sprintf(tmp, "%s/%s, %s/%s",
...
#if CODE_CHECK_IMPL
  sprintf(case_info_mm->fuzz_stat.havoc, "%s", tmp);
#endif
  ...
  SAYF(" stability : %s%-10s " bSTG bV "\n", (stab_ratio < 85 && var_byte_count > 40) 
       ...
       ? cMGN : cRST), tmp);
#if CODE_CHECK_IMPL
  sprintf(case_info_mm->fuzz_stat.stability, "%s", tmp);
#endif
  if (!bytes_trim_out) {
...
#if CODE_CHECK_IMPL
  sprintf(case_info_mm->fuzz_stat.trim, "%s", tmp);
#endif
...
#if CODE_CHECK_IMPL
      sprintf(case_info_mm->fuzz_stat.cpu, "cpu%03u:%3u%%", MIN(cpu_aff, 999),
              MIN(cur_utilization, 999));
#endif
    } else {
...
#if CODE_CHECK_IMPL
      sprintf(case_info_mm->fuzz_stat.cpu, "cpu:%3u%%",
              MIN(cur_utilization, 999));
#endif
   }
  ...
  while (1) {
    if (case_info_mm->status == READY) {
      ...
      if (case_info_mm->op == PAUSE_FUZZER) {
        ...
        case_info_mm->status = TASK_FINISHED;
      } else if (case_info_mm->op == WRITE_QUEUE_ENTRY) {
        load_queue_entry_from_case_info_mm_by_id();
        case_info_mm->status = TASK_FINISHED;
      } else if (case_info_mm->op == STOP_FUZZER) {
        handle_stop_sig(0);
        case_info_mm->status = TASK_FINISHED;
      } else if (case_info_mm->op == SKIP_CUR_CASE) {
        handle_skipreq(0);
        case_info_mm->status = TASK_FINISHED;
      } else if (case_info_mm->op == FUZZ_TARGET_CASE) {
        target_fuzz_case_idx = case_info_mm->modify_queue_entry_idx;
        handle_skipreq(0);
        case_info_mm->status = TASK_FINISHED;
      }
      continue;
...
#if CODE_CHECK_IMPL
  // 如果用户为这个种子设置了分数，我们就使用用户设定的分数
  if (queue_cur->user_set_perf_score > 0) {
    orig_perf = perf_score = queue_cur->user_set_perf_score;
  }
#endif
...
int main(int argc, char** argv) {
  ...
  while (1) {
...
#if CODE_CHECK_IMPL
    if (target_fuzz_case_idx != -1 && queue_cur) {
      // 当前是用户指定要fuzz的模式，此时我们就要将该种子的优先级设为最高
      target_fuzz_case_idx = -1;
      queue_cur->favored = 1;
    ...
    #if CODE_CHECK_IMPL
    if (target_fuzz_case_idx == -1) {
      queue_cur = queue_cur->next;
      current_entry++;
    } else {
      struct queue_entry *tmp_entry = queue;
      int tmp_cnt = 0;
      while (tmp_entry && tmp_cnt < target_fuzz_case_idx) {
        tmp_entry = tmp_entry->next;
        tmp_cnt++;
      }
      queue_cur = tmp_entry;
      current_entry = target_fuzz_case_idx;
    }
#else
    // 类似于将queue一整个遍历，然后在fuzz_one中，又以一定概率（最大不超过25%）fuzz一个种子，
    // fuzz过程有多个阶段，包括flip, havoc, insert,
    // arith等等，每个阶段都会生成很多我们感兴趣的种子
